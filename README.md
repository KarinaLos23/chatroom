...
curl -X POST localhost:8080/message -H 'Content-type:application/json' -d '{"message": "Samwise Gamgee", "sender": "<tokenRetrievedViaLogin>"}'
...
on heroku (current):
```
curl -X POST https://java-bootcamp-chatroom.herokuapp.com/message -H 'Content-type:application/json' -d '{"message": "Have a good day!", "sender": "karina"}'
...
curl -X POST https://java-bootcamp-chatroom.herokuapp.com/message -H 'Content-type:application/json' -d '{"message": "Samwise Gamgee", "sender": "<tokenRetrievedViaLogin>"}'
